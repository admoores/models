...
difference() {
  union() {
    ...
    cylinder(h = footHeight*2, r=holeRad+wallThickness, center=true);
    translate([0, -holeRad - wallThickness, -footHeight]) cube([insertDepth + offsetDepth, 2 * (holeRad + wallThickness), footHeight]);
    rotate([0, 0, -90]) translate([0, -holeRad - wallThickness, -footHeight]) cube([insertDepth + offsetDepth, 2 * (holeRad + wallThickness), footHeight]);
  }
